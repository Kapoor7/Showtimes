...
public class AddDeleteController {
    ...
    void btnCSubmitClicked(ActionEvent event)
    {
        ...
        int x = 0; int y = 0;
        try{
             x = Integer.parseInt(txtX.getText().replaceAll("\\s", ""));
             y = Integer.parseInt(txtY.getText().replaceAll("\\s", ""));
        }
        catch (NumberFormatException e){
            showAlertWithHeaderText("Please enter valid coordinates.");
            return;
        }
